...
for(cnt in seq(from=5,to=500,by=1))
{
  ...
  Scenario_3_cost[cnt1] = sum(Cost_df$Programs_Downtime_Cost_Per_Unit_Time * Cost_df$Time_To_Transfer)+sum(Cost_df$Remote_Access_Cost_Per_Unit_Time * Cost_df$Min_Remote_Time)
...
library(ggplot2)
df <- data.frame(Method=c("Method-1"), Cost=Scenario_1_cost, Data_Size = Total_Tables_size)
df = rbind(df,data.frame(Method=c("Method-2"), Cost=Scenario_2_cost, Data_Size = Total_Tables_size))
df = rbind(df,data.frame(Method=c("Method-3"), Cost=Scenario_3_cost, Data_Size = Total_Tables_size))
